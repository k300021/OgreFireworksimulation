...
Ogre::Real OgreRecast::CountPath(Ogre::Vector3 startPos, Ogre::Vector3 endPos, int nPathSlot, int nTarget)
{
    float start[3];
    float end[3];
    OgreVect3ToFloatA(startPos, start);
    ...
    int ans = FindPath(start,end,nPathSlot,nTarget);
    if( ans >= 0 )
    {
        int nVertCount=m_PathStore[nPathSlot].MaxVertex;
        Ogre::Real distance = 0;
        for( int nVert=0 ; nVert < nVertCount - 1 ; nVert++ )
            distance += Ogre::Vector3( m_PathStore[nPathSlot].PosX[nVert], 0, m_PathStore[nPathSlot].PosZ[nVert] ).distance( Ogre::Vector3( m_PathStore[nPathSlot].PosX[nVert + 1], 0, m_PathStore[nPathSlot].PosZ[nVert + 1] ) );
        return distance;
    }
    else
        return ans;
...
// TODO maybe cache generated colors so when rebuilding tiles the same colors can be reused? If possible
   ...
   */
   ...
   if(m_nAreaCount)
   {
      ...
      m_pRecastMOWalk->setQueryFlags( 1u << 1 ); 
         ...
         if (areas[i] == SAMPLE_POLYAREA_GROUND || areas[i] == DT_TILECACHE_WALKABLE_AREA)
         {
            ...
            for (int j = 2; j < nvp; ++j) // go through all verts in the polygon
            {
               ...
               for (int k = 0; k < 3; ++k) // create a 3-vert triangle for each 3 verts in the polygon.
               {
                  ...
                  if(colorRegions) {
                      m_pRecastMOWalk->colour( Ogre::Real( 37 / 255 ), Ogre::Real( 166 / 255 ), Ogre::Real( 255 / 255 ), 0.2 );  // Assign vertex color
                  } else {
...
float OgreRecast::getCellHeight()
{
    return m_cellHeight;
}
...
{
    return m_navMeshOffsetFromGround * 5;
}
